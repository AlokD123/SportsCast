...
#ASSUMING DATA IS PRESENT IN DATA_DIR

#TODO: make class instance variable (OR variable of singleton object)
...
def generate_list_ds(data, targets, targets_meta, targets_raw, stat_cat_features, dyn_cat_features, dyn_real_features, dyn_real_features_meta, player_names:list, boolTransformed=False):
...
def load_data_listDS(data,
    ...
    if ret is None: #If any errors in preparing, skip
        print(f'Error getting data for all players!')
        return None
    ...
    if boolSplitTrainTest:
        ...
        train_list_ds = generate_list_ds(train,targets_trn, targets_meta_trn, targets_raw_trn, stat_cat_features_trn, dyn_cat_features_trn, dyn_real_features_trn, dyn_real_features_meta_trn, player_names, boolTransformed=boolTransformed)
...
def load_data_main(data_path,roster_path,full_save_dir, boolSplitTrainTest, \
    ...
    try:
        ...
        full_roster = pd.read_csv(roster_path)
